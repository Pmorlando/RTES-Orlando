...
void set_scheduler(void)
{
    ...
    CPU_ZERO(&cpuset);
    cpuidx=(3); // CPU core 3 
    CPU_SET(cpuidx, &cpuset); // making CPU core 3 do all the work
    ...
    printf("ADJUSTED "); print_scheduler(); // outputs the schedule set 
}
...
void *counterThread(void *threadp)
{
    ...
    // gets current time so the compute time is logged
    ...
    // gets final time to see compute time 
...
void *starterThread(void *threadp) // gets thread attributes for first thread to sharethe other 63
{
   int i, rc;
