...
#include <sys/types.h>
#include <sched.h>
#include <unistd.h> // needed this header for getpid() to work
...
{
    int schedType = sched_getscheduler(getpid());// default linux schedule is CFS
...
void set_scheduler(void)
{
    ...
    CPU_ZERO(&cpuset);
    cpuidx=(3); //setting CPU core 3 
    CPU_SET(cpuidx, &cpuset);
    pthread_attr_setaffinity_np(&fifo_sched_attr, sizeof(cpu_set_t), &cpuset);

    max_prio=sched_get_priority_max(SCHED_POLICY); // get max sceduling priority available 
