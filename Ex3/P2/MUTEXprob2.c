...
// compiled using gcc MUTEXprob2.c -o MUTEXprob2 
...
void *writepos(void *arg)
{
  ...
  while(1) 
  {
    ...
    writeagain.tv_sec += 1; // puase for 1 sec so this will be approx 1 hz
...
void *readpos(void *arg)
{
  ...
  while(1) 
  {
    ...
    pthread_mutex_lock(&mutexsem);//lock mutex
    ...
    readagain.tv_sec += 10;// sleep for 10 sec to make 0.1 hz
