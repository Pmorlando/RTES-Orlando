...
static struct timespec logone = {0,0};
static struct timespec logtwo = {0,0};
static struct timespec logdelta = {0,0};
...
// Monotonic Raw clock chose for invariability from outside sources
...
void *delay_test(void *threadID)
{
  ...
  for(idx=0; idx < TEST_ITERATIONS; idx++)
  {
      ...
      if(idx == 10)
      {
	 clock_gettime(MY_CLOCK, &logone);
      }
      if(idx ==60) 
      {
	 clock_gettime(MY_CLOCK, &logtwo);
	 delta_t(&logtwo, &logone, &logdelta);
	 printf("time between test 10 and 60 is %ld sec and %ld nsec\n", logdelta.tv_sec, logdelta.tv_nsec);
      }
...
void end_delay_test(void)
{
    double real_dt;
#if 0
  printf("Monotonic Raw Clock start seconds = %ld, nanoseconds = %ld\n", 
         rtclk_start_time.tv_sec, rtclk_start_time.tv_nsec);
  
  printf("Monotonic Raw Clock stop seconds = %ld, nanoseconds = %ld\n", 
         rtclk_stop_time.tv_sec, rtclk_stop_time.tv_nsec);
#endif

  real_dt=d_ftime(&rtclk_start_time, &rtclk_stop_time);
  printf("Monotonic Raw Clock DT seconds = %ld, msec=%ld, usec=%ld, nsec=%ld, sec=%6.9lf\n", 
...
#endif
  printf("Monotonic Raw Clock delay error = %ld, nanoseconds = %ld\n", 
