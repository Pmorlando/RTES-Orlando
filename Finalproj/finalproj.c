...
static double spikethresh = 0.45;// test if this is an accurate thresh and increase if needed
...
static int spikethensettle(void) // if frame after diff calc spike
{
    ...
    if(spike == 1 && lastdiff < spikethresh) return 1; // 1 if there is good frame taking out   for testing
