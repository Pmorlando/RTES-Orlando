...
static double ppmtotal =0, pgmtotal =0, ppmworst = 0, pgmworst = 0;
static unsigned long pgmcount = 0, ppmcount = 0;
...
int main(int argc, char **argv)
{
    ...
    if(ppmcount >0)
    {
        syslog(LOG_INFO, "PPM write-back: n=%lu avg FPS = %.2f, worst FPS = %.2f ",
                ppmcount,
    ...
    if(pgmcount > 0)
    {
        syslog(LOG_INFO, "PGM write-back: n=%lu avg FPS = %.2f, worst FPS = %.2f ",
                pgmcount,
                1000.0 /(pgmtotal / pgmcount),
                1000.0 /pgmworst);
    }
    return 0;
}
